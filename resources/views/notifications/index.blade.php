@extends('layouts.app')
@section('title', __('站内通知'))
@section('content')
<div class="admin-shell">
  <div class="admin-inner" style="max-width:680px;">
    <div class="admin-head-row">
      <div>
        <h1 class="page-title">{{ __('站内通知') }}</h1>
        <p class="page-sub" style="margin:0;">{{ __('你所在项目 / 团队范围内的图纸变更提醒') }}</p>
      </div>
      @if($notifications->isNotEmpty())
        <form method="POST" action="{{ route('notifications.read-all') }}">
          @csrf
          <button type="submit" class="btn btn-sm">{{ __('全部标记已读') }}</button>
        </form>
      @endif
    </div>
      ...
      <form method="POST" action="{{ route('notifications.read', $n->id) }}">
        ...
        <button type="submit" class="feed-item" style="width:100%;text-align:left;border:1px solid var(--border);{{ $n->read_at ? '' : 'border-left:4px solid var(--accent);' }}">
          <div style="flex:1;min-width:0;">
            ...
            <div class="hint" style="margin-top:4px;">{{ $n->data['uploader_name'] ?? '—' }} {{ __('发布') }} · {{ $n->created_at->diffForHumans() }}</div>
          </div>
          <div class="feed-meta">
            <div class="feed-ver">{{ $n->data['version_no'] }}</div>
            @unless($n->read_at)<span class="badge-latest" style="margin-top:6px;display:inline-block;">{{ __('未读') }}</span>@endunless
          </div>
        </button>
      </form>
    @empty
      <div class="empty-state"><p>{{ __('暂时没有通知') }}</p></div>
    @endforelse
